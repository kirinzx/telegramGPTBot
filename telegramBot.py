...
async def addAccount(message:types.Message):
    ...
    await message.answer(text="Напишите номер телефона аккаунт(с кодом страны):",reply_markup=keyboardMenu)
...
async def process_nickname(message: types.Message, state: FSMContext):
    ...
    await UserForm.next()
    await message.reply("Напишите api_id")
...
async def process_app_id(message: types.Message, state: FSMContext):
    ...
    await UserForm.next()
    await message.reply("Напишите api_hash")
...
async def process_app_hash(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            ...
            client = TelegramClient(session=data["phoneNumber"],api_id=data["app_id"],api_hash=data["app_hash"])
            await client.connect()
            if not await client.is_user_authorized():
                await client.send_code_request(phone=data["phoneNumber"])
                await message.answer("Введите код, который пришел вам в телеграм!", reply_markup=keyboardMenu)
                await state.finish()

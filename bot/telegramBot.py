...
keyboardSettings = ReplyKeyboardMarkup(keyboard=[
    ["Изменить шанс комментирования",
        "Изменить время ожидания", "Изменить запрос к chatgpt"],
            ...
            if account[1] == "-":
                accountsButtons.add(InlineKeyboardButton(text=str(account[0]), callback_data=f"view {account[0]}"),
                                    InlineKeyboardButton(
                                        text="-", callback_data=f"view {account[1]}"),
                                    InlineKeyboardButton(text="Добавить", callback_data=f"Добавить гиперссылку {account[0]}"))
            else:
                accountsButtons.add(InlineKeyboardButton(text=str(account[0]), callback_data=f"view {account[0]}"),
                                    InlineKeyboardButton(
                                        text=str(account[1]), callback_data=f"view {account[1]}"),
...
async def getAccounts(message: types.Message):
    try:
        ...
        if len(accounts) > 0:
            accountsButtons = InlineKeyboardMarkup()
            for account in accounts:
                accountsButtons.add(InlineKeyboardButton(text=str(account[0]), callback_data=f"view {str(account[0])}"), InlineKeyboardButton(
...
async def process_password(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        user = User(data["phoneNumber"], data["app_id"], data["app_hash"],
                    data['ip'], data['port'], data['login'], message.text.strip())
        proxy = (python_socks.ProxyType.SOCKS5,
                 data['ip'], data['port'], True, data['login'], message.text.strip())
    try:
...
async def saveUser(message: types.Message, state: FSMContext):
    try:
        ...
        from main import usersToCheck
        usersToCheck.append(UserChecking(user.phoneNumber, user.app_id,
                            user.app_hash, user.ip, user.port, user.login, user.password))
